...
public:
    ...
    void set_first_var(int n){first_var = n;}
    int get_first_var(){return first_var;}
          ...
          case IDENT:
              if ( isalpha(c) || isdigit(c) || (c == ':') || (c == '.')){
                  ...
                  if (struct_var != -1){
                      ...
                      delete []cur_tmp_1;
                      TND[tmptmptmp].set_first_var(first_j);
                      cout << "PUT VAR" << tmptmptmp << TND[tmptmptmp].get_first_var() << endl;
                  ...
                  special_var = TND.struct_search(buf);
                  cout << "SPECIAL " << special_var << endl;
...
void Parser::STRUCT_E(){
    Stack < int, 100 > st_int_cur;
    int amount_of_assigns = 0;
    st_int_cur.push(TND[special_var].get_struct_number());
    prog.put_lex (Lex (LEX_ID, special_var));
    gl();
    while (c_type == LEX_ASSIGN){
        gl();
        if (c_type != LEX_ID){
        ...
        //cout << "SPECIALLLL " << special_var << endl;
        if (special_var == (-1)){
            throw "error in assignment with struct";
        }
        amount_of_assigns++;
        st_int_cur.push(TND[special_var].get_struct_number());
        prog.put_lex (Lex (LEX_ID, special_var));
    ...
    cout << "GG " << amount_of_assigns << endl;

    int cur_prev = st_int_cur.pop();
    int tmp_prev;    
    for (int ip = 0; ip < amount_of_assigns; ip++){
        cout << "GG " << amount_of_assigns << endl;
        if (cur_prev != (tmp_prev = st_int_cur.pop())){
            throw "Error: wrong types in assignment";
        }
        cur_prev = tmp_prev;
        prog.put_lex (Lex (LEX_ASSIGN));
    }
    ...
    switch ( pc_el.get_type () )
    {
      ...
      case LEX_ASSIGN:
        if (struct_assign_in == 1){
            i = args.pop();
            j = args.pop();
            for (int klop = 0; klop < 4; klop++){
                cout << "IIIII" << i << endl;
                if (TID[TND[i].get_first_var() + klop].get_assign()){
                    TID[TND[j].get_first_var()+klop].put_assign();
                    TID[TND[j].get_first_var()+klop].put_value(TID[TND[i].get_first_var()+klop].get_value());
                }
            }
...
main(int argc, char **argv)
{
    ...
    try{
        ...
        for (int i = 0; i < (TND.get_top()); i++){
            ...
            cout << TND[i].get_struct_number() << endl;
        }
